...
function MainScreen() {
    ...
    const [housePrice, setHousePrice] = React.useState( JSON.parse(localStorage.getItem('houseValue') || '0'));
    ...
    useEffect(() => {
        ...
        if (!Number.isNaN(anualCashFlowNumber) && !Number.isNaN(initialCostNumber)) {
            var firstYearCosts = (initialCostNumber + Number(anualCosts) + (Number(monthlyBankPayment) + Number(monthlyCosts) + Number(condominiumCosts)) * 12 + Number(imiCosts))
            var roiNumber = RoundToTwoDecimalPlaces(anualCashFlowNumber * 100 / firstYearCosts)
            ...
            var repRoiNumber = RoundToTwoDecimalPlaces((anualCashFlowNumber + RoundToTwoDecimalPlaces(monthlyBankRepaymentNumber * 12)) * 100 / firstYearCosts)
            setRepRoi(repRoiNumber.toString())
        }
    }, [anualCashFlow, anualCosts, condominiumCosts, initialCost, imiCosts, monthlyCosts, monthlyBankPayment, monthlyBankRepayment])
    ...
    useEffect(() => {
        ...
        localStorage.setItem('houseValue', String(housePriceNumber))
    ...
    return (
        ...
        <>
            ...
            <div className='roundedBox settings'>
                <form className='demoForm'>
                    <div className='form-group'>
                        ...
                        <div className="labelSpacing">
                            <label htmlFor='oneTimeCosts labelSpacing'>Custos Burocráticos
                                (IMT/IS/Escritura/Crédito)</label>
                            <div className="input-group">
                                <input type='text'
                                       className='form-control'
                                       name='oneTimeCosts'
                        ...
                        <OverlayTrigger
                            overlay={<Tooltip id="button-tooltip">
                                O Custo anual do
                                condominio {Number.isNaN(Number(condominiumCosts)) ? "" : "(" + RoundToTwoDecimalPlaces(Number(condominiumCosts) * 12) + ")"} deve
                                ser declarado no anexo F da declaração de IRS para abater nos lucros das rendas e pagar
                                menos impostos.
                            </Tooltip>}
                            placement="top"
                        ...
                        <OverlayTrigger
                            overlay={<Tooltip id="button-tooltip">
                                O IMI deve ser declarado no anexo F da declaração de IRS para abater nos lucros das
                                rendas e pagar menos impostos.
                            </Tooltip>}
                            placement="top"
                            delay={{show: 250, hide: 300}}
                        >
                        ...
                        <div className="labelSpacing">
                            ...
                            <Form.Select name='houseLocation' onChange={(e) => onChangeHouseLocation(e)}
                                         value={houseLocation}>
                        ...
                        <div className="labelSpacing">
                            ...
                            <Form.Select name='houseType' onChange={(e) => onChangeHouseType(e)} value={houseType}>
                                <option value={HouseType.HabitacaoPropriaPermanente}>Habitação própria e permanente
                                </option>
                                <option value={HouseType.HabitacaoSecundariaOuArrendamento}>Habitação Secundária
                                </option>
            ...
            <div className='roundedBox'>
                ...
                {Number(debt) > 0 && <>Custos de avaliacao de crédito 400€<br/>Custo de escritura com crédito 700€ (no
                    portal casa pronta)<br/></>}
                ...
                Total
                = {calculateIMT(houseLocation, houseType, Number(housePrice)) + RoundToTwoDecimalPlaces(Number(housePrice) * 0.008) + (Number(debt) > 0 ? 1100 : 375)}€<br/>
            </div>
        </>
    )
}
